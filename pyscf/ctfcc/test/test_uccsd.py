...
class KnownValues(unittest.TestCase):
    ...
    def test_uccsd_frozen(self):
        # Freeze 1s electrons
        frozen = [[0,1], [0,1]]
        ucc = uccsd.UCCSD(mf, frozen=frozen)
        ucc.diis_start_cycle = 1
        ecc, t1, t2 = ucc.kernel()
        self.assertAlmostEqual(ecc, -0.07414978284611283, 6)

    def test_vector_to_amplitudes(self):
        t1, t2 = mycc.vector_to_amplitudes(mycc.amplitudes_to_vector(mycc.t1, mycc.t2))
        self.assertAlmostEqual(abs(t1[0]-mycc.t1[0]).to_nparray().max(), 0, 12)
        self.assertAlmostEqual(abs(t1[1]-mycc.t1[1]).to_nparray().max(), 0, 12)
        self.assertAlmostEqual(abs(t2[0]-mycc.t2[0]).to_nparray().max(), 0, 12)
        self.assertAlmostEqual(abs(t2[1]-mycc.t2[1]).to_nparray().max(), 0, 12)
        self.assertAlmostEqual(abs(t2[2]-mycc.t2[2]).to_nparray().max(), 0, 12)

    def test_mbpt2(self):
        myucc = uccsd.UCCSD(mf)
        e = myucc.kernel(mbpt2=True)[0]
        self.assertAlmostEqual(e, -0.096257842171487, 6)
    ...
    def test_ccsd_t(self):
        ...
        et= mycc.ccsd_t_slow()
        et1= mycc.ccsd_t()
        self.assertAlmostEqual(et, -0.0010176027505768013, 6)
        self.assertAlmostEqual(et1, -0.0010176027505768013, 6)

    def test_ipccsd(self):
        mycc = uccsd.UCCSD(mf)
        mycc.kernel()
        e, v = mycc.ipccsd(nroots=4)
        self.assertAlmostEqual(e[0], 0.15082821014476724, 6)
        self.assertAlmostEqual(e[1], 0.27128474878779196, 6)
        self.assertAlmostEqual(e[2], 0.4594230554796196, 6)
        self.assertAlmostEqual(e[3], 0.6233541619027493, 6)
    ...
    def test_eaccsd(self):
        ...
        self.assertAlmostEqual(e[0], -0.09881085219916441, 6)
        self.assertAlmostEqual(e[1], 0.094440274855227, 6)
        self.assertAlmostEqual(e[2], 0.1739431181202621, 6)
        self.assertAlmostEqual(e[3], 0.1956993723111896, 6)
